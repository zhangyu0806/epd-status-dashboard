...
class LunarCalendarWidget(Widget):
    def render(self, ctx: RenderContext, rect: Rect) -> None:
        ...
        day_text = f"{now.day}"
        ctx.draw.text((rect.x + 6, rect.y + 2), day_text, fill=RED, font=day_font)
        dw = text_width(ctx.draw, day_text, day_font)

        right_x = rect.x + 14 + dw
        ctx.draw.text((right_x, rect.y + 2), f"{now.year % 100:02d}年{now.month}月", fill=BLACK, font=label_font)
        ctx.draw.text((right_x, rect.y + 22), weekday_name(now), fill=RED, font=label_font)
